...
parser.add_argument('--model', type=str, default='trained_models/ae_atlasnet_25.pth',
...
parser.add_argument('--gen_points', type=int, default=2500,
                    help='number of points to generate, put 30000 for high quality mesh, 2500 for quantitative comparison with the baseline')
...
with torch.no_grad():
    for i, data in enumerate(dataloader_test, 0):
        ...
        b[:, 1:] = np.array(faces)
        write_ply(filename=opt.model[:-4] + "/" + str(dataset_test.cat[cat]) + "/" + fn + "_gen" + "_grain_" + str(
            int(opt.gen_points)),
                  points=pd.DataFrame(torch.cat((pointsReconstructed.cpu().data.squeeze(), grid_pytorch), 1).numpy()),
