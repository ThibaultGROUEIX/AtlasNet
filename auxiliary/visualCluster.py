...
def main(trainer, outHtml=None):
    ...
    webpage.add_title(f"Reconstruction (Val):{loss_val}")

    table = webpage.add_table()
    table2 = deepcopy(table)
    table2.add_titleless_columns(1)
    table.add_titleless_columns(2)
    curve_recons = webpage.chart(trainer.data_curve, title="Reconstruction quality - chamfer log")
    ...
    if trainer.epoch > 5 and trainer.opt.primitive_selection:
